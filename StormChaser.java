...
public static Storm GetStorm( Scanner in ) 
{
   // Build a Storm object and return it
	...
	beginDate = year * 10000 + month * 100 + day; //ex. YYYYMMDD
	duration = 6;
	...
	while( in.hasNextLine() && current == sequence ) 
	{
            ...
            //get next record (Same as record above (year, month, day...))
            year = in.nextInt();
            month = in.nextInt();
            day = in.nextInt();
            skip = in.nextInt();
            sequence = in.nextInt();
            name = in.next();
            junk = in.nextDouble(); //ones for longitude
            junk = in.nextDouble(); //ones for latitude
            wind = in.nextInt();
            pressure = in.nextInt();                                   		               
